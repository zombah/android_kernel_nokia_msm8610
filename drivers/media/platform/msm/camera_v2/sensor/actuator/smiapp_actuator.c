...
/* Autofocus (AF) registers (read only) */ 
#define CAM_SMIApp_REG8_ACTUATOR_CAPABILITY         0x1B04
#define CAM_SMIApp_REG16_ACTUATOR_TYPE              0x1B40
#define CAM_SMIApp_REG8_AF_DEVICE_ADDRESS           0x1B42
#define CAM_SMIApp_REG16_FOCUS_CHANGE_ADDRESS       0x1B44
...
/* These are read from the sensor (SMIA specifies the location) */
static uint16_t af_device_address              = 0x00;
static uint16_t actuator_type                 = 0x0000;
static uint16_t focus_change_address          = 0x0000;

/* focus_change_control_address is calculated from focus_change_address (defined in SMIA) */
/* These registers could be either in the sensor itself or in a separate actuator chip. */
static uint16_t focus_change_control_address          = 0x0000;
	...
	int16_t next_lens_position, uint32_t hw_params, uint16_t delay)
{
	...
	CDBG("Enter\n");
		...
		i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr = focus_change_address;
		i2c_tbl[a_ctrl->i2c_tbl_index].reg_data = next_lens_position >> 8; //high byte
		i2c_tbl[a_ctrl->i2c_tbl_index].delay = 0;
        CDBG("i2c_byte1:0x%x, i2c_byte2:0x%x\n", i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr,i2c_tbl[a_ctrl->i2c_tbl_index].reg_data);
		a_ctrl->i2c_tbl_index++;
        i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr = focus_change_address+1;
		i2c_tbl[a_ctrl->i2c_tbl_index].reg_data = next_lens_position & 0x00FF; //low byte
		i2c_tbl[a_ctrl->i2c_tbl_index].delay = 0;
        CDBG("i2c_byte1:0x%x, i2c_byte2:0x%x\n", i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr,i2c_tbl[a_ctrl->i2c_tbl_index].reg_data);
		a_ctrl->i2c_tbl_index++;

        		i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr = focus_change_control_address;
		i2c_tbl[a_ctrl->i2c_tbl_index].reg_data = 0x02; //high byte
		i2c_tbl[a_ctrl->i2c_tbl_index].delay = 0;
        CDBG("i2c_byte1:0x%x, i2c_byte2:0x%x\n", i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr,i2c_tbl[a_ctrl->i2c_tbl_index].reg_data);
		a_ctrl->i2c_tbl_index++;

        		i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr = focus_change_control_address+1;
		i2c_tbl[a_ctrl->i2c_tbl_index].reg_data = 0x01; //low byte
		i2c_tbl[a_ctrl->i2c_tbl_index].delay = delay;
        CDBG("i2c_byte1:0x%x, delay:%d\n", i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr,i2c_tbl[a_ctrl->i2c_tbl_index].delay);
		a_ctrl->i2c_tbl_index++;
...
static void msm_toshiba_raisu_actuator_parse_i2c_params(struct msm_actuator_ctrl_t *a_ctrl,
	int16_t next_lens_position, uint32_t hw_params, uint16_t delay)
{
#define  VCM_CODE_MSB (0x3093)
#define  VCM_CODE_LSB (0x3094)
#define  VCM_SHDW_CNTRL (0x3080)
	...
	CDBG("Enter\n");
	CDBG("next_lens_position :%d\n", next_lens_position);
	i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr = VCM_CODE_MSB;
	i2c_tbl[a_ctrl->i2c_tbl_index].reg_data = next_lens_position >> 8; //high byte
	i2c_tbl[a_ctrl->i2c_tbl_index].delay = 0;
	CDBG("i2c_byte1:0x%x, i2c_byte2:0x%x\n", i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr,i2c_tbl[a_ctrl->i2c_tbl_index].reg_data);
	a_ctrl->i2c_tbl_index++;

	i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr = VCM_CODE_LSB;
	i2c_tbl[a_ctrl->i2c_tbl_index].reg_data = next_lens_position & 0x00FF; //low byte
	i2c_tbl[a_ctrl->i2c_tbl_index].delay = 0;
	CDBG("i2c_byte1:0x%x, i2c_byte2:0x%x\n", i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr,i2c_tbl[a_ctrl->i2c_tbl_index].reg_data);
	a_ctrl->i2c_tbl_index++;

	i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr = VCM_SHDW_CNTRL;
	i2c_tbl[a_ctrl->i2c_tbl_index].reg_data = 0x07;
	i2c_tbl[a_ctrl->i2c_tbl_index].delay = 0;
	CDBG("i2c_byte1:0x%x, i2c_byte2:0x%x\n", i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr,i2c_tbl[a_ctrl->i2c_tbl_index].reg_data);
	a_ctrl->i2c_tbl_index++;
...
static void msm_toshiba_raisu_actuator_set_pwm_mode(struct msm_actuator_ctrl_t *a_ctrl, uint16_t delay)
{
	struct msm_camera_i2c_reg_setting reg_setting;
	struct msm_camera_i2c_reg_array *i2c_tbl = a_ctrl->i2c_reg_tbl;
	int32_t rc = 0;
	CDBG("Enter\n");
	a_ctrl->i2c_tbl_index = 0;
	i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr = 0x3084;
	i2c_tbl[a_ctrl->i2c_tbl_index].reg_data = 0x1c;
	i2c_tbl[a_ctrl->i2c_tbl_index].delay = 0;
	a_ctrl->i2c_tbl_index++;

	i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr = 0x3085;;
	i2c_tbl[a_ctrl->i2c_tbl_index].reg_data = 0x06;
	i2c_tbl[a_ctrl->i2c_tbl_index].delay = 0;
	a_ctrl->i2c_tbl_index++;

	i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr = 0x3086;
	i2c_tbl[a_ctrl->i2c_tbl_index].reg_data = 0x11;
	i2c_tbl[a_ctrl->i2c_tbl_index].delay = 0;
	a_ctrl->i2c_tbl_index++;

	i2c_tbl[a_ctrl->i2c_tbl_index].reg_addr = 0x3087;
	...
	int16_t code_boundary)
{
    ...
    CDBG("damping_code_step:%d, wait_time:%d,code_boundary:%d\n",
        damping_code_step, wait_time,code_boundary);
	...
	struct msm_actuator_set_position_t *set_pos)
{
	...
	if (set_pos->number_of_steps  == 0)
		return rc;
	if(manufacturer_id == CAM_SMIApp_MANUFACTURER_TOSHIBA
		&& chipid== CAM_SMIApp_MODEL_TOSHIBA_RAISU){
		is_toshiba_raisu = 1;
		msm_toshiba_raisu_actuator_set_pwm_mode(a_ctrl, 20000);
	}
	if(is_toshiba_raisu){
		actuator_parse_i2c_params = msm_toshiba_raisu_actuator_parse_i2c_params;
	}else{
		actuator_parse_i2c_params = a_ctrl->func_tbl->actuator_parse_i2c_params;
	}
	CDBG("%s Enter is_toshiba_raisu %d,id:%04x,%02x\n", __func__, is_toshiba_raisu,manufacturer_id,chipid);
...
static int32_t msm_actuator_init(struct msm_actuator_ctrl_t *a_ctrl,
	...
	a_ctrl->curr_region_index = 0;
      ...
      a_ctrl->i2c_client.i2c_func_tbl->i2c_read(
            &a_ctrl->i2c_client,
            CAM_SMIApp_REG8_ACTUATOR_CAPABILITY,
            &actuator_type, MSM_CAMERA_I2C_BYTE_DATA);
    CDBG("CAM_SMIApp_REG8_ACTUATOR_CAPABILITY:0x%x,\n", actuator_type);
    
    a_ctrl->i2c_client.i2c_func_tbl->i2c_read(
            &a_ctrl->i2c_client,
            CAM_SMIApp_REG16_ACTUATOR_TYPE ,
            &actuator_type, MSM_CAMERA_I2C_WORD_DATA);
    CDBG("actuator_type:0x%x,\n", actuator_type);
    
            a_ctrl->i2c_client.i2c_func_tbl->i2c_read(
            &a_ctrl->i2c_client,
            CAM_SMIApp_REG8_AF_DEVICE_ADDRESS,
            &af_device_address, MSM_CAMERA_I2C_BYTE_DATA);
    CDBG("af_device_address:0x%x,\n", af_device_address);
    
            a_ctrl->i2c_client.i2c_func_tbl->i2c_read(
            &a_ctrl->i2c_client,
            CAM_SMIApp_REG16_FOCUS_CHANGE_ADDRESS,
            &focus_change_address, MSM_CAMERA_I2C_WORD_DATA);        
    focus_change_control_address = focus_change_address+2;
    CDBG("focus_change_address:0x%x,\n", focus_change_address);
...
static const struct i2c_device_id msm_actuator_i2c_id[] = {
	{"qcom,smiapp_actuator", (kernel_ulong_t)NULL},
	{ }
};
...
static const struct of_device_id msm_actuator_dt_match[] = {
	{.compatible = "qcom,smiapp_actuator", .data = NULL},
	{}
};
