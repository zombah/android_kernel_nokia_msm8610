...
#ifdef WORK_QUEUE
	struct workqueue_struct *spi_work_wq;
	struct work_struct spi_work_work;
#else	
...
{	
#ifdef WORK_QUEUE
	struct _spi_device_st *spi_device = container_of(arg, struct _spi_device_st, spi_work_work);
#else	
...
#ifdef WORK_QUEUE
		queue_work(spi_device->spi_work_wq,&spi_device->spi_work_work);
#else
...
static int sms_keep_alive_thread(void *context)
{
	...
	for (;;)
	{
		...
		msleep(50);
		if (!smsspiphy_is_device_exists(&smsspi_pdata))
		{
...
static int sms3130_parse_dt(struct platform_device *pdev)
{
	int rc;

	rc = of_property_read_string(pdev->dev.of_node, "sms,vdd1_supply", &smsspi_pdata.vdd1);
	if (!rc)
		sms_info("vdd1_supply = %s\n", smsspi_pdata.vdd1);
	else
		sms_err("read vdd1_supply error\n");

	rc = of_property_read_string(pdev->dev.of_node, "sms,vdd2_supply", &smsspi_pdata.vdd2);
	if (!rc)
		sms_info("vdd2_supply = %s\n", smsspi_pdata.vdd2);
	else
		sms_err("read vdd2_supply error\n");

	rc = of_property_read_u32(pdev->dev.of_node, "sms,rst_gpio", &smsspi_pdata.rst_gpio);
	if (!rc)
		sms_info("rst_gpio = %d\n", smsspi_pdata.rst_gpio);
	else
		sms_err("read rst_gpio error\n");

	rc = of_property_read_u32(pdev->dev.of_node, "sms,pow1_gpio", &smsspi_pdata.pow1_gpio);
	if (!rc)
		sms_info("pow1_gpio = %d\n", smsspi_pdata.pow1_gpio);
	else
		sms_err("read pow1_gpio error\n");

	rc = of_property_read_u32(pdev->dev.of_node, "sms,pow2_gpio", &smsspi_pdata.pow2_gpio);
	if (!rc)
		sms_info("pow2_gpio = %d\n", smsspi_pdata.pow2_gpio);
	else
		sms_err("read pow2_gpio error\n");

	rc = of_property_read_u32(pdev->dev.of_node, "sms,irq_gpio", &smsspi_pdata.irq_gpio);
	if (!rc)
		sms_info("irq_gpio = %d\n", smsspi_pdata.irq_gpio);
	else
		sms_err("read irq_gpio error\n");

	rc = of_property_read_u32(pdev->dev.of_node, "sms,spi_bus_num", &smsspi_pdata.spi_bus_num);
	if (!rc)
		sms_info("spi_bus_num = %d\n", smsspi_pdata.spi_bus_num);
	else
		sms_err("read spi_bus_num error\n");

	rc = of_property_read_u32(pdev->dev.of_node, "sms,spi_cs_num", &smsspi_pdata.spi_cs_num);
	if (!rc)
		sms_info("spi_cs_num = %d\n", smsspi_pdata.spi_cs_num);
	else
		sms_err("read spi_cs_num error\n");

	rc = of_property_read_u32(pdev->dev.of_node, "sms,spi_max_speed", &smsspi_pdata.spi_max_speed);
	if (!rc)
		sms_info("spi_max_speed = %d\n", smsspi_pdata.spi_max_speed);
	else
		sms_err("read spi_max_speed error\n");

	rc = of_property_read_u32(pdev->dev.of_node, "sms,spi_download_speed", &smsspi_pdata.spi_download_speed);
	if (!rc)
		sms_info("spi_download_speed = %d\n", smsspi_pdata.spi_download_speed);
	else
		sms_err("read spi_download_speed error\n");

	return 0;
}

static int sms3130_spi_probe(struct platform_device *pdev)
{
	sms_info("sms3130_spi_probe enter\n");
	if (!pdev->dev.of_node) {
		sms_err("SMS driver only supports device tree probe\n");
		return -ENOTSUPP;
	}

	sms3130_parse_dt(pdev);

	return 0;
}

static struct of_device_id sms3130_spi_of_match[] = {
	{ .compatible = "sms,sms3130_spi_adapter", }, { }
};

MODULE_DEVICE_TABLE(of, sms3130_spi_of_match);
EXPORT_COMPAT("sms,sms3130_spi_adapter");

static struct platform_driver sms3130_spi_driver = {
	.driver = {
		.name = "sms3130_spi_adapter",
		.owner = THIS_MODULE,
		.of_match_table = sms3130_spi_of_match,
	},
	.probe = sms3130_spi_probe,
};
...
int smsspi_register(void)
{
...
#ifdef WORK_QUEUE
	spi_device->spi_work_wq = create_singlethread_workqueue("siano work queue");
	INIT_WORK(&spi_device->spi_work_work, spi_worker_thread);
#else
	INIT_WORK(&spi_device->work_thread, spi_worker_thread);
#endif	
	INIT_WORK(&spi_device->connect_thread, smsspi_connect);

	spi_device->chip_model = 0x3130;//0;
	...
	ret = platform_driver_register(&sms3130_spi_driver);
	if (ret) {
		pr_err("register sms driver failed!\n");
		return ret;
	}
	...
	spi_device->phy_dev = smsspiphy_register(&smsspi_pdata,
						smsspi_int_handler, 
						spi_device);
...
void smsspi_unregister(void)
{
	...
	smsspi_deinit_device(spi_device);
	smsspiphy_unregister(spi_device->phy_dev, &smsspi_pdata);
...
#ifdef WORK_QUEUE
	destroy_workqueue(spi_device->spi_work_wq);
#endif
	kzfree(spi_device);
	platform_device_unregister(&smsspi_device);
	platform_driver_unregister(&sms3130_spi_driver);
	sms_info("exiting\n");
}
