...
static int default_mode = SMSHOSTLIB_DEVMD_ISDBT;  //5
...
int smscore_start_device(void* dev)
{
	...
	schedule_delayed_work(&coredev->work_queue, 20);
#ifdef SMS_RC_SUPPORT_SUBSYS
...
static int smscore_load_firmware_from_file(struct smscore_device_t *coredev,
	...
	if (!sms_static_firmware)
	{
		...
		sms_static_firmware = vmalloc(ALIGN(fw->size + sizeof(sms_static_firmware), SMS_ALLOC_ALIGNMENT));
		if (!sms_static_firmware) {
...
char *smscore_get_fw_filename(void* dev, int mode, int lookup) {
	...
	// if get unknow type, return "none"
	if ((type == SMS_UNKNOWN_TYPE) || 
	   (mode >= SMSHOSTLIB_DEVMD_MAX))
	{
		return smscore_fw_lkup[default_mode][0];
	}
