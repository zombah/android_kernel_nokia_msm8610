...
struct cyttsp4_core_driver {
	...
	bool (*request_fw_status)(struct cyttsp4_device *ttsp);
	...
	void (*get_touch_record)(struct cyttsp4_device *ttsp, int rec_no,
			int *rec_abs);
	void (*request_set_power_state)(struct cyttsp4_device *ttsp, u8 state);
...
static inline void cyttsp4_request_set_power_state(struct cyttsp4_device *ttsp, u8 state)
{
	struct cyttsp4_core *cd = ttsp->core;
	struct cyttsp4_core_driver *d = to_cyttsp4_core_driver(cd->dev.driver);
	d->request_set_power_state(ttsp, state);
}
...
static inline bool cyttsp4_request_fw_status(
		struct cyttsp4_device *ttsp)
{
	struct cyttsp4_core *cd = ttsp->core;
	struct cyttsp4_core_driver *d = to_cyttsp4_core_driver(cd->dev.driver);
	return d->request_fw_status(ttsp);
}
